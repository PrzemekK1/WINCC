...
{
	int afActive=0;
	int rc=paCfgReadValue(PROJ_PATH+"config/config", "general", "activateAlertFiltering", afActive);
	if (rc < 0) return true; // config entry does not exist -> default is TRUE
	if (afActive > 0) return true;
	return false;
}
			...
			case DPCONFIG_ALERT_NONBINARYSIGNAL:
				...
				for(j = 1; j <= diAlertRanges[i]; j++)
				{
					...
					else
					{
						...
						k++;
						if(diHysteresisType[j] == DPATTR_HYST_VALUE) //get hysteresis values
						{
...
//                         "_l_hyst_limit",ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW]);
				...
				if(dmDpeType[n] != DPDETAIL_RANGETYPE_MATCH && dmDpeType[n] != DPDETAIL_RANGETYPE_SET)
				{
					for(j = diAlertRanges[i]; j >= 2; j--)
					{
						...
						if(diHysteresisType[j - 1] != DPATTR_HYST_VALUE)
						{
		...
		dyn_bool &hysteresisType, dyn_bool &includeLimits, dyn_string &exceptionInfo)
{
	...
	for(i = 1; i <= ranges; i++)
	{
		...
		hysteresisType[i] = (bool)DPATTR_HYST_NONE;
	}
...
//compute hysteresis values
	if(ranges >= 2)
	{
		...
		if(dfHysteresisValues[iLow] != 0)
		{
			hysteresisType[1] = (bool) DPATTR_HYST_VALUE;
		}
		hysteresisLimits[2] = limits[1] + dfHysteresisValues[1];
		if(dfHysteresisValues[iUp] != 0)
		{
			hysteresisType[2] = (bool) DPATTR_HYST_VALUE;
		}
	}
	if(ranges >= 3)
	{
		...
		if(dfHysteresisValues[iUp] != 0)
		{
			hysteresisType[3] = (bool) DPATTR_HYST_VALUE;
		}
	}
	if(ranges >= 4)
	{
		...
		if(dfHysteresisValues[iUp] != 0)
		{
			hysteresisType[4] = (bool) DPATTR_HYST_VALUE;
		}
	}
	if(ranges >= 5)
	{
		...
		if(dfHysteresisValues[iUp] != 0)
		{
			hysteresisType[5] = (bool) DPATTR_HYST_VALUE;
		}
	}
