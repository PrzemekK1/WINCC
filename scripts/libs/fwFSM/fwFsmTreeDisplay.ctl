...
private const bool fwFsmTreeDisplay_fwFsmUserLibLoaded = fwGeneral_loadCtrlLib("fwFSM/fwFsmUser.ctl",false);
private const bool fwFsmTreeDisplay_fwFSMConfDBLoaded = fwGeneral_loadCtrlLib("fwFSMConfDB/fwFsmConfDB.ctl",false);
private const bool fwFsmTreeDisplay_fwAccessControlLoaded = fwGeneral_loadCtrlLib("fwAccessControl/fwAccessControl.ctc",false);
...
//  DebugN("Answer from popup: ", answer);
	if(answer == 1)
	{
	}
	else if(answer == 2)
...
//DebugTN("Removing 2", parent, node, redo, redo_nodes, redo_node_parent);
...
//		if(node != "FSM")
			redo_node_parent = node;
//DebugTN("Adding 10", parent, node, redo, redo_nodes, redo_node_parent);
	}
	else if(answer == 11)
	{
...
//		if(node != "FSM")
			redo_node_parent = node;
//DebugTN("Adding 11", parent, node, redo, redo_nodes, redo_node_parent);
	}
	else if(answer == 12)
	{
	}
...
//		if(parent != "FSM")
			redo_node_parent = parent;
//DebugTN("Adding 14", parent, node, redo, redo_nodes, redo_node_parent);
	}
	else if(answer == 15)
	{
...
//			if(parent != "FSM")
	...
	else if(answer == 100)
	{
		new_node = fwFsmTree_askRemoveNode(parent, node, redo);
		dynAppend(redo_nodes,new_node);
//DebugTN("Removing 100", parent, node, redo, redo_nodes, redo_node_parent);
	}
	if(redo)
	{
...
//				fwUi_uninformUserProgress(1);
		...
		{
			if((!fwFsm_isDomain(redo_nodes[i])) && (redo_nodes[i] != fwFsm_clipboardNodeName))
			{
...
//		parent = "";
	if(parent != node)
	{
      ...
      int ctrlFlag = fwFsm_getCUCtrlFlag(node);
      if((ctrlFlag == 1) || (ctrlFlag == 3))
      {
				dynAppend(txt,"SEPARATOR");
				dynAppend(txt,"PUSH_BUTTON, Restart Device Handler, 10, 1");
      }
		}
	}
	else
	{
		...
		dynAppend(txt,"PUSH_BUTTON, Restart SMI Handler, 8, 1");
		dynAppend(txt,"PUSH_BUTTON, Restart Device Handlers, 9, 1");
	}
	popupMenu(txt,answer);
	if(answer == 1)
	{
...
//		fwUi_showFsmObject(parent, node);
	...
	{
		fwFsmTree_restartPVSS00ctrl(node);
	}
}
...
//fwFsmTree_renameNode:
/**  Rename a node in the FSM Tree.

Usage: JCOP framework internal, public

PVSS manager usage: VISION, CTRL

      @param node: the name of the node to be renamed.
      @param name: the name of the new node.
*/

fwFsmTree_renameNode(string node, string name)
{
int i, cu;
...
/*
	fwTreeDisplay_callUser2(
		...
		node, parent);
*/
	fwTree_renameNode(node, name, exInfo);
	FSM_nodeRenamed(node, name);
/*
	fwTree_getChildren(node, children, exInfo);
	if(dynlen(children))
	{
...
//  dpSetWait("ToDo.treeRedos", FwTreeRedos[treeIndex]);
...
//  DebugN("Function: ", __FUNCTION__, "Line: ", __LINE__, "File: ", __FILE__);
	...
	if((parent != "FSM") && (parent != fwFsm_clipboardNodeName))
	{
...
//DebugTN("Gen 2");
	fwUi_informUserProgress("Please wait...", 100,60);
//DebugTN("Gen 3");
DebugTN("fwFsmTree_generateAll: Generating Device Types...");
	fwFsm_generateDeviceTypes();
//DebugTN("Gen 4");
DebugTN("fwFsmTree_generateAll: Generating Object Types...");
	fwFsm_generateObjectTypes();
//DebugTN("Gen 5");
	fwTree_getChildren("FSM",nodes, exInfo);
DebugTN("fwFsmTree_generateAll: Generating Tree Nodes...");
...
//DebugN("new_dp", old_cu, dp, res, obj, type);
	...
	obj = fwFsm_extractSystem(obj);
  if((sys != "") && (sys != fwFsm_getSystemName()))
    obj = sys+":"+obj;
/*
...
*/
//DebugTN("fwFsmTree_generateTreeNode: "+node);
//DebugTN("Gen 11");
	fwTree_getNodeCU(node, cu, exInfo);
	if(!cu)
		return;
...
int ok, i, auto_start, start_all_systems, summary_alarms, auto_include, ref_auto_enable, tree_update_mode;
	...
	if(res[1])
	{
		...
		old_sys = ret[2];
		sys = ret[3];
		auto_start = (int) ret[4];
		start_all_systems = (int) ret[5];
		summary_alarms = (int) ret[6];
   auto_include = (int) ret[7];
   ref_auto_enable = (int) ret[8];
   tree_update_mode = (int) ret[9];
		...
		if(ref_auto_enable != -1)
    {
     if(dpExists("ToDo.refAutoEnable"))
		...
		if(tree_update_mode != -1)
    {
     if(dpExists(fwTreeDisplay_getType("FSM")+".treeUpdateMode"))
			  dpSet(fwTreeDisplay_getType("FSM")+".treeUpdateMode", tree_update_mode);
    }
	}
}
...
fwFsmTree_restartPVSS00ctrl(string domain = "")
{

	fwUi_informUserProgress("Please wait...",100,60);
	fwFsm_restartPVSS00ctrl("", domain);
	fwUi_uninformUserProgress(1);
}
