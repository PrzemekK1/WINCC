...
private const bool fwFsmUi_fwFsmUserLibLoaded = fwGeneral_loadCtrlLib("fwFSM/fwFsmUser.ctl",false);
...
{
	string panel, panel_path, sys, dev, dp, tnode, type;
  dyn_string exInfo;
	...
	if(panel != "")
	{
		...
		if(panel_path != "")
		{
			...
			if(fwFsm_isDU(node, obj))
			{
				...
				dev = sys+dev;
//DebugTN("addUserpanel", node, obj, sys, dev);
        if(!dpExists(dev))
        {
        	dp = sys+node+fwFsm_separator+obj;
	        if(dpExists(dp))
	        {
		        dpGet(dp+".tnode",tnode);
            fwTree_getNodeDevice(tnode, dev, type, exInfo);
          }
        }
			}
...
*/
	myState = fwUi_convertLocalCurrentState(myDomain, myObj, state);
	if(StateWidget != "none")
		setValue(StateWidget,"text",myState);
	_fwFsmUi_show_mode();
}
...
synchronized _fwFsmUi_do_show_mode()
{
	bit32 bits;
...
//DebugTN("******* fwUi_do_show_mode",myDomain, myObj, myDelaying, getStackTrace());
	...
	fwUi_getLocalObjStateColor(myDomain, myObj, myState, color);
	if(!FSMBusy)
  {
   actionFilter = fwFsmUi_getActionFilter(myDomain, myObj);
		fwUi_getLocalObjStateActions(myDomain, myObj, myState, actions, visis, actionFilter);
//DebugTN("fwUi_show_mode",myDomain, myObj, myState, actions, visis, actionFilter);
  }
	else
	{
...
//		dynAppend(visis, 2);
	...
	if((!dynlen(actions)) && (myState == "DEAD"))
	{
		dynAppend(actions,"RestartFSM");
		dynAppend(visis, 2);
   if(isLHCb())
   {
	...
	AccessControlOperator = fwFsmUi_getOperatorAccess(myDomain, myObj);
	AccessControlExpert = fwFsmUi_getExpertAccess(myDomain, myObj);
//DebugTN("fwUi_show_mode1",myDomain, myObj, myState, actions, visis, actionFilter);
//DebugN("show_mode", myDomain, myObj, myState, color, bits, actions, visis);
//DebugTN("show_mode", myDomain, myObj, myState, color, AccessControlOperator, AccessControlExpert);
//DebugTN("_fwFsmUi_show_mode", myDomain, myObj, myState, color, bits);
	if(ColorWidget != "none")
	{
	...
	if(ActionsWidget == "none")
		return;
  if((fwFsm_isDU(myDomain, myObj)) || (fwFsm_isLU(myDomain, myObj)))
  {
    	operate = (
...
//DebugN(actions);
   ...
   else
   {
		  ...
		  if((visis[i+1] == 2) && (!AccessControlExpert))
		  	oper = 0;
//DebugN("enabling",i, myDomain, myObj, myState, actions[i+1], oper);
...
//DebugTN("*** fwUi_show_mode Starting",myDomain, myObj, myDelaying, getStackTrace());
	if(!myDelaying)
	{
		myDelaying = 1;
//		delay(0, 800);
...
fwFsmUi_startDEN(string host, int daPort, int evPort)
{
  ...
  if (_UNIX)
  {
    system(PVSS_BIN_PATH+"WCCOAui"+" "+projConfig+" "+dist+" "+panel+" -menuBar -iconBar&");
    DebugN(PVSS_BIN_PATH+"WCCOAui"+" "+projConfig+" "+dist+" "+panel+" -menuBar -iconBar&");
  }
  else
  {
    system("start /B "+PVSS_BIN_PATH+"WCCOAui"+" "+projConfig+" "+dist+" "+panel+" -menuBar -iconBar");
    DebugN("start /B "+PVSS_BIN_PATH+"WCCOAui"+" "+projConfig+" "+dist+" "+panel+" -menuBar -iconBar");
  }
}
...
//	DebugN("changeMode of "+obj);
  ...
  dpSet("_Ui_"+myManNum()+".ReturnValue.Float:_original.._value",makeDynFloat());
  dpSet("_Ui_"+myManNum()+".ReturnValue.Text:_original.._value",makeDynString());
...
//DebugTN("fwFsmUi_changeMode", show_it, fwFsm_getAssociatedObj(obj), report, report1);
	if(show_it)
	{
		fwUi_uninformUser();
...
//DebugTN("Panel close Done action", errors, lockedOut, $obj);
...
fwFsmUi_modesPanelUser(string domain = "", string object = "", string action = "")
{
  string myDp;
  dyn_string items;
  time stime;
  ...
  char manType, manNum;
  int sysNum;
  string manName, manUser, cu, obj, msg, cmd;

  manUser = "Unknown";
  manName = getSystemName()+"_fwFsm_Manager"+(int)myManNum();
  if(dpExists(manName))
    dpGet(manName+".info",manUser);
  cmd = action;
  if(cmd == "")
    getValue("","text",cmd);
  cu = domain;
  ...
  msg = "*** INFO - "+manUser+" issued "+cmd+" to "+obj;
  strreplace(msg,"&&","&");
DebugTN(msg, cu);
  fwUi_report(msg, cu);
  if(shapeExists("Message"))
    setValue("Message","items",makeDynString());
}
...
//  if(isLHCb())
  ...
  {
    fwFsmUi_modesPanelUser();
    if(shapeExists("ProgressBar.progress"))
    {
      setValue("ProgressBar.progress","text","Please be patient...");
//    setValue("ProgressBar.abort","visible",1);
      setValue("ProgressBar.abort","text","Abort");
    }
  }
...
//        setValue("","enabled",0);
  ...
  getValue("Message","items",errors);
  getValue("LockedOut","items",lockedOut);
//DebugTN("Done action", errors, lockedOut);
  dynAppend(errors, lockedOut);
  for(i = 1; i <= 7; i++)
