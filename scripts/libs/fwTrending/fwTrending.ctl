...
#uses "fwTrending/fwTrendingDeprecated.ctl"
#uses "fwTrending/fwTrendingLbExtras.ctl"
#uses "fwGeneral/fwScreenShot.ctl"
#uses "fwTrending/fwTrendingTree.ctl"
#uses "unDistributedControl/unDistributedControl.ctl"

//#uses "fwAccessControl/fwAccessControl.ctc" // NOT NEEDED: dynamic weak loading below - see _fwTrending_tryLoadFwAC()

// conditionally load fwAccessControl lib, if exists (ie if installed)
private const bool _fwTrending_fwACLoaded = _fwTrending_tryLoadFwAC();

private bool _fwTrending_tryLoadFwAC()
{
	return fwGeneral_loadCtrlLib("fwAccessControl/fwAccessControl.ctc",false);
}
...
const int   fwTrending_ALARM_LIMIT_PERCENT           = 5; // Default value used to increase limits to display the alarm ranges.
...
// Keeping FULLCAPTION panel without layout for backwards compatibility (especially for cases where this panel is embedded).
// The newly introduced fork with layout is going to be used (overriding the normal one) in the functions where trend
// opens in new window (eg. dynamic trend)
const string fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION = "objects/fwTrending/fwTrendingFaceplateFullCaption.pnl";
const string fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION_WLAYOUT = "objects/fwTrending/fwTrendingFaceplateFullCaption_wLayout.pnl";
...
const dyn_string fwTrending_LAYOUTED_PANELS = makeDynString(fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION_WLAYOUT,
                                                            fwTrending_PANEL_PLOT_ZOOMED);
...
//CG
  if(maxCols)
	  data[fwTrending_PAGE_OBJECT_NCOLS] = maxCols;
  if(maxRows)
	  data[fwTrending_PAGE_OBJECT_NROWS] = maxRows;
//CG end
...
void fwTrending_newPlot(string sPlotName, string sModel = fwTrending_YT_PLOT_MODEL)
{
  ...
  else
  {
    ...
    ddsData[fwTrending_PLOT_OBJECT_BACK_COLOR]           = "_Transparent";
...
{
	bool isLog, grid, legend, configDpExists, allAuto, storedRanges, bPlot_AutoScale;
	int i, iRes;
  ...
  // Checking if auto scale is selected for all the curves
  allAuto = TRUE;
  ...
  for( i = 1 ; ((i <= fwTrending_TRENDING_MAX_CURVE) && allAuto) ; i++ )
	{
    if( _fwTrending_isAutoLimits(split[i] + " : " + split2[i], bPlot_AutoScale)  == FALSE )
    {
      fwException_raise(exceptionInfo, "ERROR", "Error checking the the max and min limits to enable autoscale in plot number: " + i + ", Max and min limits were: " + split[i] + " : " + split2[i], "");
      fwExceptionHandling_display(exceptionInfo);
      return;
    }
    else
    {
      if( bPlot_AutoScale == FALSE )
      {
        allAuto = FALSE;
      }
    }
...
//	setValue(ref + "autoScaleCheckBox", "enabled", storedRanges || !allAuto);
  if(!isFunctionDefined("fwTrending_isLHCb"))
  	setValue(ref + "autoScaleCheckBox", "enabled", storedRanges || !allAuto);
  else
  	setValue(ref + "autoScaleCheckBox", "enabled", true);
//CG end
...
{
	bool isConnected, bValidCurve = TRUE, bConnectDPE = TRUE, bConstantLine, isNumericDpe = true;
	int i, j, k;
		...
		else
		{
			...
			else
			{
				...
				if(!dpExists(dsCurveDPE[i]))
				{
					bConnectDPE = FALSE;
					isConnected = false;
					dsCurveLegend[i] += getCatStr("fwTrending", "NOTEXIST");
					curveColor = "FwTrendingDataNoAccess";
					dsCurveToolTip[i] += getCatStr("fwTrending", "NOTEXIST");
					fwException_raise(exceptionInfo, "INFO", dsCurveDPE[i] + getCatStr("fwTrending", "NOTEXIST"), "");
				}
				else
				{
					/* start 20/05/2008: Herve */
					...
					if(bConnectDPE)
					{
						...
						if(!isConnected)
						{
							dsCurveLegend[i] += getCatStr("fwTrending", "REMOTEUNAVAILABLE");
							curveColor = "FwTrendingDataNoAccess";
							dsCurveToolTip[i] += getCatStr("fwTrending", "REMOTEUNAVAILABLE");
						}
						else
						{
                            ...
                            //FWTREND-973 this is not longer an issue, code removed - START
							...
							//		isConnected = false;
							//		/* end 20/05/2008: Herve */
							//		dsCurveLegend[i] += getCatStr("fwTrending", "LOG0");
							//		curveColor = "FwTrendingDataNoAccess";
							//		dsCurveToolTip[i] += getCatStr("fwTrending", "LOG0");
							//		fwException_raise(exceptionInfo, "INFO", dsCurveDPE[i] + getCatStr("fwTrending", "LOG0"), "");
							//	}
							//}
                            //FWTREND-973 this is not longer an issue, code removed - END
						}
					}
					else
					{
            ...
            {
    						fwException_raise(exceptionInfo, "INFO", dsCurveDPE[i] + " does not hold a numeric value (boolean, integer, or real) and cannot be trended", "");
            }
					}
				}
			}
...
{
	if(dynlen(value) < 2){
		return; // Alarm Handling not configured for DPE, exit immediately
	}
...
@param sRef             input,  reference of the trend
@param ddsPlotData      input,  detailed information of the plot
@param sTrendName       input,  name of the trend for which to get the shape
@param exceptionInfo    output, the exception is returned here
*/
fwTrending_trendUpdateScales(string sRef, dyn_dyn_string ddsPlotData, string sTrendName, dyn_string exceptionInfo)
{
  bool bIsLogYAxis, bIsLinked, bAreLimitsShown, bIsAutoScale;
  int iLoop, iNumberCurves;
  float fCurrentMax, fMinForLog, fMaxPercentForLog;
  string sCurveName;
  shape shActiveTrend;
  dyn_bool dbAutoScaleRange;
  dyn_float dfLogLimitMin, dfLogLimitMax;
  dyn_string dsRanges, dsLinkedRanges, dsCurvesDpe, dsMinMaxRanges;


  fMinForLog         = ddsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_FOR_LOG]           [1];
  fMaxPercentForLog  = ddsPlotData[fwTrending_PLOT_OBJECT_EXT_MAX_PERCENTAGE_FOR_LOG][1];
  dsCurvesDpe        = ddsPlotData[fwTrending_PLOT_OBJECT_DPES];
  dsMinMaxRanges     = ddsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE];
  shActiveTrend      = getShape(sTrendName);


  // Check if curves are logaritmic
  ...
  iNumberCurves = dynlen(dsCurvesDpe);
  for( iLoop = 1 ; iLoop <= iNumberCurves ; iLoop++ )
  {
    // Check if autoscale is enable, for that we need to check all curves looking for max min = "0 : 0" or similar... If not range found we enable autoscale for the curve
    if( dsMinMaxRanges[iLoop] != "" )
    {
      dsRanges = strsplit(dsMinMaxRanges[iLoop], ":");
      sscanf(dsRanges[1], "%f", dfLogLimitMin[iLoop]);
      sscanf(dsRanges[2], "%f", dfLogLimitMax[iLoop]);


      // In case of logarithmic scale Y-min = 1e-1 and Y-max = Max + (Max*5/100) to be able to see properly the log -> Looks like old case, to be reviewed.
      if( bIsLogYAxis )
      {
        if( dfLogLimitMin[iLoop] <= 0 )
        {
          dfLogLimitMin[iLoop] = fMinForLog;
        }
        dfLogLimitMax[iLoop] += dfLogLimitMax[iLoop] * fMaxPercentForLog / 100.0;
      }


      // If lower limit > higher limit then enable autoscale
      if( dfLogLimitMin[iLoop] >= dfLogLimitMax[iLoop] )
      {
        dbAutoScaleRange[iLoop] = TRUE;
      }
      else
      {
        dbAutoScaleRange[iLoop] = FALSE;
      }
    }
    else
    {
    ...
    // If Dpe to show exists,
    if( dpExists(dpSubStr(dsCurvesDpe[iLoop], DPSUB_SYS_DP_EL)) )
    {
      // Check if curve is linked with some other
      if( (ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK][iLoop] != 0) &&     // Not linked setting
          (ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK][iLoop] != iLoop) )  // Linked with itself
      {
        bIsLinked = TRUE;
      }
      else
      ...
      // Check if alarm limits needs to be shown
      if( (ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]        == "TRUE") ||
          (ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW][iLoop] == TRUE)     )
      {
        bAreLimitsShown = TRUE;
      }
      else
      {
        bAreLimitsShown = FALSE;
      }


      // Check if autoscale is selected
      if( ! _fwTrending_isAutoLimits(dsMinMaxRanges[iLoop], bIsAutoScale) )
      {
        fwException_raise(exceptionInfo, "ERROR", "fwTrending_trendUpdateScales(): Error invalid range interval for curve " + iLoop + ": " + dsMinMaxRanges[iLoop], "");
      }


      // Set the autoscale settings
      sCurveName = "curve_" + iLoop;
      shActiveTrend.curveAutoscale(sCurveName, dbAutoScaleRange[iLoop]);


      // Calculate properly axis Min/Max ranges

        // Use the user limits
      if( !bIsAutoScale && !bIsLinked )
      {
        dsRanges = strsplit(dsMinMaxRanges[iLoop], ":");
        shActiveTrend.curveMin(sCurveName, dsRanges[1]);
        shActiveTrend.curveMax(sCurveName, dsRanges[2]);
      }


        // Use the linked curve limits
      if( bIsAutoScale && bIsLinked )
      {
        dsLinkedRanges = strsplit(dsMinMaxRanges[ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK][iLoop]], ":");
        shActiveTrend.curveMin(sCurveName, dsLinkedRanges[1]);
        shActiveTrend.curveMax(sCurveName, dsLinkedRanges[2]);
      }


        // Use the alert limits
      if( bIsAutoScale && bAreLimitsShown )
      {
        fwTrending_trendZoomToAlarmLimits(sRef);
      }


        // Use the calculated logaritmic limits
      if( !bIsAutoScale && bIsLogYAxis )
      {
        fCurrentMax = shActiveTrend.curveMax(sCurveName);
        if( dfLogLimitMin[fwTrending_Y_VALUE] >= fCurrentMax )
        {

          shActiveTrend.curveMax(sCurveName, dfLogLimitMax[fwTrending_Y_VALUE]);
          shActiveTrend.curveMin(sCurveName, dfLogLimitMin[fwTrending_Y_VALUE]);
        }
        else
        {
          shActiveTrend.curveMin(sCurveName, dfLogLimitMin[fwTrending_Y_VALUE]);
  ...
  // Autoscale for XY plots
  if( dynlen(dbAutoScaleRange) == fwTrending_X_VALUE )
  {
    shActiveTrend.curveAutoscaleX(sCurveName, dbAutoScaleRange[fwTrending_X_VALUE]);
    if( !dbAutoScaleRange[fwTrending_X_VALUE] )
    {
      fCurrentMax = shActiveTrend.curveMaxX(sCurveName);
      if(dfLogLimitMin[fwTrending_X_VALUE] >= fCurrentMax)
      {
        shActiveTrend.curveMaxX(sCurveName, dfLogLimitMax[fwTrending_X_VALUE]);
        shActiveTrend.curveMinX(sCurveName, dfLogLimitMin[fwTrending_X_VALUE]);
      }
      else
      {
        shActiveTrend.curveMinX(sCurveName, dfLogLimitMin[fwTrending_X_VALUE]);
                               ...
                               bool passingCurveVisible = FALSE)
{
  ...
  int i, timeRange, refXPos, refYPos, oldXPos, oldYPos, markerType, iCurveScalePos;
  shape activeTrendShape, standardTrendShape;
  string ref, cleanRef, activeTrend;
  dyn_string ranges, curvesList, dsParentShape;
  ...
  {
      // We print the diagnostic message already in fwTrending_showStandardTrend
      return;
  }
  ...
  // FWTREND-1048 - Start
  //   Set background of the panel to the background selected, as shapes are mostly transparent
  dsParentShape = strsplit(trendShapeStr[fwTrending_ACTIVE_TREND_NAME], ".");
  if( dynlen(dsParentShape) > 1 )
  {
    string bgCol=plotExtendedObject[fwTrending_PLOT_OBJECT_BACK_COLOR][1];
    // Special treatment for transparent background: do not reset the back color,
    // otherwise trend buttons will turn black
    if (bgCol!="_Transparent") {
      setValue(dsParentShape[1], "backCol", bgCol);
    }
  }
  else
  {
    fwException_raise(exceptionInfo, "ERROR", "fwTrending_initTrendWithObject(): Error getting parent shape of: " + trendShapeStr[fwTrending_ACTIVE_TREND_NAME] + " found " + dynlen(dsParentShape) + " items: " + dsParentShape, "");
    fwExceptionHandling_display(exceptionInfo);
  }
  // FWTREND-1048 - End
  ...
  {
    standardTrendShape.timeFormat(0,
                                  FALSE,
                                  plotExtendedObject[fwTrending_PLOT_OBJECT_AXII_X_FORMAT][1],
  ...
  _fwTrending_convertFrameworkToPvssMarkerType(plotExtendedObject[fwTrending_PLOT_OBJECT_MARKER_TYPE][1],
                                               markerType,
                                               exceptionInfo);

  for( i = 1 ; i <= fwTrending_TRENDING_MAX_CURVE ; i++ )
  {
    standardTrendShape.curveScaleBackCol("curve_" + i,
                                         plotExtendedObject[fwTrending_PLOT_OBJECT_BACK_COLOR][1]);
  ...
  else if( shapeExists(ref + "trendCaption.allowMoving") )
  {
    ...
    if(!fwTrending_isPanelLayoutEnabled())
    { // In a panel with layout enabled the code below would not have any effect
...
bool fwTrending_isPanelLayoutEnabled()
{
    shape sh = self;
    string shapeTypeName;
    getValue(sh, "shapeType", shapeTypeName);
    while(shapeTypeName != "PANEL" && sh != 0){
        getValue(sh, "panel", sh);
        getValue(sh, "shapeType", shapeTypeName);
    }
    bool isLayoutEnabled;
    getValue(sh, "layoutEnabled", isLayoutEnabled);
...
/** Show / hide the control menu bar
...
@param visible		input, visibility state of the control bar.
					...
					2: control bar visible, caption bar visible // (caption bar hidden not implemented)
					3: control bar hidden, caption bar visible // (caption bar hidden not implemented)
*/
fwTrending_controlBarOnOff(string ref, int visible)
{
	...
	dyn_string exceptionInfo;
	string buttonIcon;
	string isRunning, sCurrentPanelFileName;
	dyn_string plotShapes, plotData;
	shape standardTrendShape;
	string trendShapeName;
	bool bControlBarVisible, bLayoutedWindow;
	//bool bCaptionBarVisible;
	string cmdRef, refForRuntime;

  // FWTREND-1097 - Flag to skip coordinates setup for layouted panels
  sCurrentPanelFileName = panelFileName(myModuleName(), myPanelName());
  // VOY AQUI
  bLayoutedWindow       = (dynContains(fwTrending_LAYOUTED_PANELS, sCurrentPanelFileName) == 0 )  ? false : true;
    ...
    // FWTREND-1097  - Flag to skip coordinates for layouted panels
	// Handle faceplate with big legend else handle faceplate with small legend
	if(shapeExists(cmdRef + "Text8") && !bLayoutedWindow)
	{
	...
	if (sCurrentPanelFileName.endsWith("cpcFaceplateController.pnl")) {
		setValue("trendCaption", "position", refX, 315);
	}
	...
	switch(visible)
	{
		case 3://no bars visible - trend fully stretched
			getValue(cmdRef + "currentTime", "position", iLegendX, iLegendY);
			trendY = refY;
			int origTrendW, origTrendH;
			getValue(trendShapeName, "origSize", origTrendW, origTrendH);
			refX = trendX - 2 * (trendW/origTrendW);
			//trendH = bottomY - trendY;
			trendH = iLegendY - trendY;
			buttonIcon = "[pattern,[fit,png,expand_icon.png]]";
		...
		case 2://control bar visible
			...
			//trendH = bottomY - trendY;
			trendH = iLegendY - trendY;
			buttonIcon = "[pattern,[fit,png,collapse_icon.png]]";
  ...
  //FWTREND-1097 - In case it is a layouted window behave differently
  // added additional check if layout is not enabled in a panel
  // as bLayoutedWindow is not always correct
  if( !bLayoutedWindow && !fwTrending_isPanelLayoutEnabled() )
  {
    setMultiValue( trendShapeName,               "size",     trendW,             trendH,
  ...
  {
    if(shapeExists(ref + "LAYOUT_GROUP2"))
    {
      setValue(ref + "LAYOUT_GROUP2", "visible", bControlBarVisible);
    }
    else
    {
      shape controlBarShape;
      getValue(ref + "TimeCascadeButton", "ownerRef", controlBarShape);
      setValue(controlBarShape, "visible", bControlBarVisible);
    }
  }


  //FWTREND-1097 - In case it is a layouted window with CloseButton there is a need to adjust alignment
  if( shapeExists(ref + "toggleControlBar") )
  {
    setValue(ref + "toggleControlBar", "fill",    buttonIcon);
    setValue(ref + "toggleControlBar", "visible", true);
    if( bLayoutedWindow && shapeExists("CloseButton") )
    {
      if( bControlBarVisible )
      {
        setValue(ref + "toggleControlBar", "layoutAlignment", "AlignTop");
      }
      else
      {
        setValue(ref + "toggleControlBar", "layoutAlignment", "AlignRight");
      }
    }
  }
...
fwTrending_showStandardTrend(bool visibility, string standardStr, string logStr, dyn_dyn_string plotData = "")
{
	...
	// NOTE! the shape that is passed could be the trend plot, but
	// in case of any initialization fault it will be the "standardTrendText" static text field(!)
	// hence it would not have certain properties and would drop plenty of errors (FWTREND-1009)
	// For this reason we need to check the shape type:
	...
	if(standardTrendShape.shapeType == "TREND")
	{
		...
		DebugTN("ERROR",__FUNCTION__,myModuleName(),myPanelName(),"the type of standardTrendShape \'"+standardStr+"\'"+
		         " is not TREND;it is "+standardTrendShape.shapeType,"likely a initialization/configuration problem");
	}
}
...
fwTrending_showPlot(int iNumberOfPlots, dyn_string dsPlotName, dyn_string &dsReference, dyn_string templateParameters)
{
	...
	string sCurrentPanelFileName, sTrendPanelName, controls, panelType;
    ...
    if ( "" == dsPlotName[i] )
    {
      continue;
		...
		if(dsPlotName[i] != "")
		{
			...
			// Flag to distinguish between Trending plots and pages with or without layout
			sCurrentPanelFileName = panelFileName(myModuleName(), myPanelName());

			if(strpos(sCurrentPanelFileName,"noLayout") >= 0) {
				...
				addSymbol(myModuleName(), myPanelName(), controls, dsRef[i], iTrendControlPosY[i], iTrendControlPosX[i], "LAYOUT_GROUP0",
						1, 1, makeDynString("$sRefName:" + dsRef[i], "$templateParameters:" + templateParameters[i]));

				addSymbol(myModuleName(), myPanelName(), sTrendPanelName + panelType + ".pnl", dsRef[i] + "trend",
						iTrendPosY[i], iTrendPosX[i], "LAYOUT_GROUP0", 1, 1, dsCurveParameters);
			}
		}
	}
...
_fwTrending_gotoNow(shape trendName, int range)
{
	...
	//    else
	if(trendName.shapeType == "TREND") trendName.gotoNow();
}
...
@param sReference    input, reference of the trend shape
*/
fwTrending_trendZoomToAlarmLimits(string sReference)
{
  bool bAlertExists, bAlertActive;
  int iAlertType, iIndex, iLoop, iLen;
  float fLimitMin, fLimitMax, fAbsolute;
  shape shTrendShape;
  string sTrendRunning, sAlertPanel, sAlertHelp, sCurrentParameterValues, sCurveDpeName;
  dyn_float dfAlertLimits;
  dyn_string dsPlotShapes, exceptionInfo, dsAlertTexts, dsAlertClasses, dsSummaryDpeList, dsAlertPanelParameters;
  dyn_dyn_float ddfAlertLimitsList;
  ...
  fwTrending_getRuntimePlotDataWithExtendedObject(sReference, sTrendRunning, dsPlotShapes, ddsPlotData, exceptionInfo, FALSE);

  // Prepare plot for display
  _fwTrending_preparePlotObjectForDisplay(ddsPlotData, sCurrentParameterValues, exceptionInfo);

  // Get active trend shape name
  shTrendShape = getShape(dsPlotShapes[fwTrending_ACTIVE_TREND_NAME]);


  iIndex = 1;
  iLen   = dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_DPES]);
  for( iLoop = 1 ; iLoop <= iLen ; iLoop++ )
  {

    if( ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW][iLoop] )
    {
      sCurveDpeName = dpSubStr(ddsPlotData[fwTrending_PLOT_OBJECT_DPES][iLoop], DPSUB_SYS_DP_EL);
      if( dpExists(sCurveDpeName) )
      {
        fwAlertConfig_get(sCurveDpeName, bAlertExists, iAlertType, dsAlertTexts, dfAlertLimits, dsAlertClasses, dsSummaryDpeList, sAlertPanel, dsAlertPanelParameters, sAlertHelp, bAlertActive, exceptionInfo);
      }

      ddfAlertLimitsList[iIndex] = dfAlertLimits;
      iIndex++;

      if( dynlen(ddfAlertLimitsList) > 0 )
      {
        fLimitMin = _fwTrending_getMin(ddfAlertLimitsList);
        fLimitMax = _fwTrending_getMax(ddfAlertLimitsList);
        fAbsolute = fabs((fLimitMax - fLimitMin) / fwTrending_ALARM_LIMIT_PERCENT);

        shTrendShape.curveMin("curve_" + iLoop, fLimitMin - fAbsolute);
        shTrendShape.curveMax("curve_" + iLoop, fLimitMax + fAbsolute);
      }
    }
  }

}// fwTrending_trendZoomToAlarmLimits()
...
{
  int iLoop, iLenOrdered, iLenDPE, iLoopInt, iLoopCount, iBonus;
  file fFile;
  ...
  for( iLoop = 1 ; iLoop <= iLenDPE ; iLoop++ )
  {
    ...
    {
      dpGetPeriod(dtStartInt[iLoop], dtEndInt[iLoop], iBonus, sTempDPE + ":_offline.._value", dfValue, dtOrdered);
  ...
  for( iLoopInt = 1 ; iLoopInt <= iLenOrdered ; iLoopInt++ )
  {
    ...
    // process samples outside the currently visible time range
    if( (dtOrdered[iLoopInt] < tFirstTime) || (dtOrdered[iLoopInt] > tLastTime) )
    {
      for( iLoop = 1 ; bFillEmptyCells && iLoop <= iLenDPE ; iLoop++ )
      {
...
//CG sort faster
	...
	{
    last = 0;
    for(j = 1; j <= dynlen(ddt[i]); j++)
		{
			if(period(ddt[i][j]) != last)
			{
				last = period(ddt[i][j]);
				ddt[i][j] = last;
			}
...
fwTrending_openPlotDisplay(string sPlotDpName, string sPanelFileName, bool bAsNewModule, bool bStayOnTopOrModal,
                           string sModuleName, string sPanelName,
                           unsigned x = 0,     unsigned y = 0,        string sResizeMode = "")
{
  ...
  switch( sPanelFileName )
  {
    ...
    case fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION_WLAYOUT:
  ...
  // Forcing FACEPLATE_FULLCAPTION with layout for not embedded trend
  if (sPanelFileName == fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION) {
    sPanelFileName = fwTrending_PANEL_PLOT_FACEPLATE_FULLCAPTION_WLAYOUT;
  }
...
@param 	iTagIndex	index corresponding to the Plot configuration attribute.
...
{
  if( (iTagIndex > fwTrending_SIZE_PLOT_OBJECT) ||
      (iTagIndex < 1)                            )
  {
    return FALSE;
  }
...
@param 	iTagIndex	index corresponding to the Page configuration attribute.
...
{
  if( (iTagIndex > fwTrending_SIZE_PAGE_OBJECT) ||
      (iTagIndex < 1)                            )
  {
    return FALSE;
  }
...
void fwTrending_openConfiguration(string sRef, dyn_string dsPlotData)
{
  ...
  int iLength, iLoop, iOriginalTime;
  time tStart, tEnd;
  string sTrendingRunning, sCurrentParameterValues, sDpName;
  shape shActiveTrendShape;
  dyn_float dfReturn;
  dyn_string dsReturn, dsPlotShapes, dsPanels, dsModifiedPlotData, dsOldCurves, dsVisibleCurves, exceptionInfo;
  dyn_dyn_string ddsExtendedPlotData;


  // DebugTN("fwTrending_openConfiguration() -> sRef = ",       sRef);
  // DebugTN("fwTrending_openConfiguration() -> dsPlotData = ", dsPlotData);

  getValue(sRef + "trend.dpNameText", "text", sDpName);
  // DebugTN("fwTrending_openConfiguration() -> sDpName = " + sDpName);

  fwTrending_convertStringToDyn(dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], dsVisibleCurves, exceptionInfo);
  // DebugTN("fwTrending_openConfiguration() -> dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN] = ", dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN]);
  // DebugTN("fwTrending_openConfiguration() -> dsVisibleCurves = ",                                  dsVisibleCurves);
  // DebugTN("fwTrending_openConfiguration() -> exceptionInfo = ",                                    exceptionInfo);

  _fwTrending_switchCurvesHiddenVisible(dsVisibleCurves, exceptionInfo);
  // DebugTN("fwTrending_openConfiguration() -> _fwTrending_switchCurvesHiddenVisible exceptionInfo = ",                                    exceptionInfo);
  ...
  // DebugTN("fwTrending_openConfiguration() -> fwTrending_PLOT = ", fwTrending_PLOT);
  // DebugTN("fwTrending_openConfiguration() -> dsPanels = ",        dsPanels);
  // DebugTN("fwTrending_openConfiguration() -> exceptionInfo = ",   exceptionInfo);

  // FWTREND-964,FWTREND-1050
  //   Get start time, end time and time configuration setting from Trend Widget and pass to the panel as $ parameter and compare it after panel returns.
  iOriginalTime = dsPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE];
  getValue(sRef + "trend.standardTrend", "visibleTimeRange", 0, tStart, tEnd);
  // DebugTN("fwTrending_openConfiguration() -> iOriginalTime = ", iOriginalTime);
  // DebugTN("fwTrending_openConfiguration() -> tStart = ", tStart);
  // DebugTN("fwTrending_openConfiguration() -> tEnd   = ", tEnd);
  // END FWTREND-964,FWTREND-1050


  ChildPanelOnModalReturn(dsPanels[1] + ".pnl",
                          "Plot Configuration",
                          makeDynString("$Command:onlineedit",
                                        "$sDpName:"       + sDpName,
                                        "$dsPlotData:"    + dsPlotData,
                                        "$dsShowButtons:" + makeDynString("ok", "cancel") ),
                          0, 0,
                          dfReturn, dsReturn);
  // DebugTN("fwTrending_openConfiguration() -> dsReturn = ", dsReturn);
  // DebugTN("fwTrending_openConfiguration() -> dfReturn = ", dfReturn);


  if( dsReturn[1] != "Cancel" )
  {
    // DebugTN("fwTrending_openConfiguration() -> ! Cancel");

    strreplace(dsReturn[3], " | ", "|");
    // DebugTN("fwTrending_openConfiguration() -> dsReturn[3] = ", dsReturn[3]);


    dsModifiedPlotData = strsplit(dsReturn[3], "|");
    // DebugTN("fwTrending_openConfiguration() -> dsModifiedPlotData = ", dsModifiedPlotData);


    for( iLoop = 1 ; iLoop <= fwTrending_SIZE_PLOT_OBJECT ; iLoop++ )
    {
      dsModifiedPlotData[iLoop] = strltrim(dsModifiedPlotData[iLoop], " ");
      dsModifiedPlotData[iLoop] = strrtrim(dsModifiedPlotData[iLoop], " ");
    }
    // DebugTN("fwTrending_openConfiguration() -> dsModifiedPlotData after spaces = ", dsModifiedPlotData);


    fwTrending_convertStringToDyn(dsModifiedPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], dsVisibleCurves, exceptionInfo);
    // DebugTN("fwTrending_openConfiguration() -> dsModifiedPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN] = ", dsModifiedPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN]);
    // DebugTN("fwTrending_openConfiguration() -> dsVisibleCurves = ",                                          dsVisibleCurves);
    // DebugTN("fwTrending_openConfiguration() -> exceptionInfo = ",                                            exceptionInfo);
    ...
    setValue(sRef + "trend.trendInfo", "items", dsModifiedPlotData);
    // DebugTN("fwTrending_openConfiguration() -> set plot config ->  setValue(sRef + \"trend.trendInfo\", \"items\", dsModifiedPlotData)");


    fwTrending_getRuntimePlotDataWithExtendedObject(sRef, sTrendingRunning, dsPlotShapes, ddsExtendedPlotData, exceptionInfo, FALSE);
    // DebugTN("fwTrending_openConfiguration() -> sRef = ",                sRef);
    // DebugTN("fwTrending_openConfiguration() -> sTrendingRunning = ",    sTrendingRunning);
    // DebugTN("fwTrending_openConfiguration() -> dsPlotShapes = ",        dsPlotShapes);
    // DebugTN("fwTrending_openConfiguration() -> ddsExtendedPlotData = ", ddsExtendedPlotData);
    // DebugTN("fwTrending_openConfiguration() -> exceptionInfo = ",       exceptionInfo);


    shActiveTrendShape = getShape(dsPlotShapes[fwTrending_ACTIVE_TREND_NAME]);
    // DebugTN("fwTrending_openConfiguration() -> shActiveTrendShape = ", shActiveTrendShape);


    fwTrending_convertStringToDyn(dsPlotData[fwTrending_PLOT_OBJECT_DPES], dsOldCurves, exceptionInfo);
    // DebugTN("fwTrending_openConfiguration() -> dsPlotData[fwTrending_PLOT_OBJECT_DPES] = ", dsPlotData[fwTrending_PLOT_OBJECT_DPES]);
    // DebugTN("fwTrending_openConfiguration() -> dsOldCurves = ",                             dsOldCurves);
    // DebugTN("fwTrending_openConfiguration() -> exceptionInfo = ",                           exceptionInfo);


    iLength = dynlen(dsOldCurves);
    // DebugTN("fwTrending_openConfiguration() -> iLength = ", iLength);


    if(iLength > fwTrending_TRENDING_MAX_CURVE)
    {
      iLength = fwTrending_TRENDING_MAX_CURVE;
      // DebugTN("fwTrending_openConfiguration() -> iLength set to: " + fwTrending_TRENDING_MAX_CURVE);
    }


    // In case conf panel has replaced / removed some signals
    for( iLoop = 1 ; iLoop <= iLength ; iLoop++ )
    {
      if( dsOldCurves[iLoop] != ddsExtendedPlotData[fwTrending_PLOT_OBJECT_DPES][iLoop] )
      {
        // DebugTN("fwTrending_openConfiguration() -> curve " + iLoop + " signals has been modified");
        // DebugTN("fwTrending_openConfiguration() -> curve " + iLoop + " -> before: " + dsOldCurves[iLoop]);
        // DebugTN("fwTrending_openConfiguration() -> curve " + iLoop + " -> now   : " + ddsExtendedPlotData[fwTrending_PLOT_OBJECT_DPES][iLoop]);

        shActiveTrendShape.disconnectDirectly("curve_" + iLoop);
        shActiveTrendShape.curveRemoveData   ("curve_" + iLoop, getCurrentTime());
      }
    }


    // In case of logarithmic Y scale signals
    ...
    // DebugTN("fwTrending_openConfiguration() -> plot has Y axis logaritmic? " + bIsLog);


    setValue(sRef + "trend.activeTrendText", "text", dsPlotShapes[fwTrending_LINEAR_TREND_NAME]);
    // DebugTN("fwTrending_openConfiguration() -> set shape \"trend.activeTrendText\", \"text\", to: ", dsPlotShapes[fwTrending_LINEAR_TREND_NAME]);

    // DebugTN("fwTrending_openConfiguration() -> dsPlotShapes before = ", dsPlotShapes);
    dsPlotShapes[fwTrending_ACTIVE_TREND_NAME] = dsPlotShapes[fwTrending_LINEAR_TREND_NAME];
    // DebugTN("fwTrending_openConfiguration() -> dsPlotShapes after = ", dsPlotShapes);


    getValue(sRef + "trend.parameterValues", "text", sCurrentParameterValues);
    // DebugTN("fwTrending_openConfiguration() -> sCurrentParameterValues = ", sCurrentParameterValues);


    fwTrending_checkAndGetAllTemplateParameters(sRef, ddsExtendedPlotData, fwTrending_PLOT, sCurrentParameterValues, exceptionInfo, TRUE);
    // DebugTN("fwTrending_openConfiguration() -> sRef = ",                    sRef);
    // DebugTN("fwTrending_openConfiguration() -> ddsExtendedPlotData = ",     ddsExtendedPlotData);
    // DebugTN("fwTrending_openConfiguration() -> fwTrending_PLOT = ",         fwTrending_PLOT);
    // DebugTN("fwTrending_openConfiguration() -> sCurrentParameterValues = ", sCurrentParameterValues);
    ...
    setValue(sRef + "trend.parameterValues", "text", sCurrentParameterValues);
    // DebugTN("fwTrending_openConfiguration() -> set shape \"trend.parameterValues\", \"text\", with value: ", sCurrentParameterValues);


    _fwTrending_preparePlotObjectForDisplay(ddsExtendedPlotData, sCurrentParameterValues, exceptionInfo);

    getValue(sRef + ".saveSettings", "enabled", bEnabled);
    fwTrending_initTrendWithObject(sRef, dsPlotShapes[fwTrending_ACTIVE_TREND_NAME], dsPlotShapes, ddsExtendedPlotData, sTrendingRunning, TRUE, exceptionInfo, TRUE);
    ...
    fwTrending_trendUpdateScales(sRef, ddsExtendedPlotData, dsPlotShapes[fwTrending_ACTIVE_TREND_NAME], exceptionInfo);
    fwTrending_initControlTrendButtons(ddsExtendedPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE][1], sRef, sDpName, ddsExtendedPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC][1], false);

    // connect to alarm handling for those dps showing the alarm limits on the trend
    _fwTrending_connectToAlarmHandling(sRef, ddsExtendedPlotData);
    setValue(sRef + ".saveSettings", "enabled", bEnabled);


    // FWTREND-964, FWTREND-1050, FWTREND-1073
    //   Recover time window if no changes has been done in time settings
    // DebugTN("fwTrending_openConfiguration() -> iOriginalTime = ", iOriginalTime);
    // DebugTN("fwTrending_openConfiguration() -> dsModifiedPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE] = ", dsModifiedPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE]);
    if( iOriginalTime == dsModifiedPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE] )
    {
      setValue(sRef + "trend.standardTrend", "visibleTimeRange", 0, tStart, tEnd);
    }
    // END FWTREND-964, FWTREND-1050, FWTREND-1073
...
@param ref the trending shape name
@param dsPlotData the trending data
@param bCanUserEditPlot indicates that the user saving the plot also has edit privileges on it from fwAccessControl
...
fwTrending_saveAs(string ref, dyn_string dsPlotData, bool bCanUserEditPlot = true)
{
    ...
    if ( false == bCanUserEditPlot ) // user cannot edit plot, so remove access control in saved DP
    {
      dyn_errClass err;

      dpSetWait(dsReturn[2] + fwTrending_PLOT_ACCESS_CONTROL_SAVE, "");

      err = getLastError(); //test whether an error occurred
      if(dynlen(err) > 0)
      {
        fwException_raise(exceptionInfo, "WARNING", "Could not remove the acces control for the data point " + dsReturn[2] + ".", "");
        fwExceptionHandling_display(exceptionInfo);
      }
...
/**@brief Check if Auto scale is selected
...
@param sMinMaxRange,     input, string with the following format: "MinLimit : MaxLimit". For example: "0 : 0", "0.000 : 0.000"
@param bIsAuto,          output, if auto limit is selected

@return boolean,         output, FALSE if range format is invalid
*/
bool _fwTrending_isAutoLimits(string sMinMaxRange, bool &bIsAuto)
{
  float fMaxLimit, fMinLimit;
  dyn_string dsLimits;

  bIsAuto  = FALSE;
  dsLimits = strsplit(sMinMaxRange, ":");


  // Check limits range format
  if( dynlen(dsLimits) != 2 )
  {
    // Range format is invalid: autolimits disabled
    return FALSE;
  }


  fMinLimit = (float)dsLimits[1];
  fMaxLimit = (float)dsLimits[2];


  if( (fMinLimit == 0.0) &&
      (fMaxLimit == 0.0 ) )  // Autoscale selected if Min and Max limits are 0
  {
    bIsAuto = TRUE;
  }
...
@reviewed 2019-02-12 @whitelisted{FalsePositive}
